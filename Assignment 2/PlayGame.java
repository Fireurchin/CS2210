...
public class PlayGame extends JFrame {
	...
	/*
	 * To run the program type: java PlayGame size to_win, where size is the size of
	 * the board and to_win is the number of symbols in line needed to win the game.
	 */
	...
	private class ClickHandler implements ActionListener {
		...
		/*
		 * When the user has selected a play, this method is invoked to process the
		 * selected play
		 */
		...
		/*
		 * Displays the play made by the computer. If the computer takes a long time a
		 * message is displayed so the humanplayer knows that the program is still
		 * working
		 */
		...
		private PosPlay computerPlay(char symbol, int highest_score, int lowest_score, int level) {
			...
			if (++numCalls == THINKING) {
				...
				isThinking = true;
			} else if ((numCalls % THINKING) == 0)
				System.out.print(".");

			// Check if the next move wins or loses the game. This step is added to speed-up
			// the algorithm
			if (level == 0) {
			...
			for (int r = 0; r < boardRows; r++) {
				...
				for (int c = 0; c < boardCols; c++) {
					if (t.squareIsEmpty(row, column)) { // Empty position
						...
						} else if ((symbol == COMPUTER && reply.getScore() == COMPUTER_LOSES
								&& reply.getLevel() > bestLevel)) {
							bestRow = row;
							bestColumn = column;
							bestLevel = reply.getLevel();
							value = reply.getScore();
