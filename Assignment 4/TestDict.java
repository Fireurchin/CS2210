...
public class TestDict {
	...
	public static void main(String[] args) {
		...
		String definitions[] = { "Very enjoyable work that students need to complete outside the classroom.",
				"A series of talks or lessons. For example, CS210.", "A set of students taught together,",
				"An electronic machine frequently used by Computer Science students.", "One more than three" };
		Key[] keys = new Key[5];
		...
		try {
			...
			if (rec == null)
				...
				System.out.println("Test 4 failed" + " " + rec.getKey().getName());
		} catch (DictionaryException e) {
		...
		try {
			...
			if ((rec.getKey().getName()).compareTo(words[3]) == 0)
				...
				System.out.println(
						"Test 7 failed" + " " + rec.getKey().getName() + ", " + words[3] + ", " + keys[2].getName());
		} catch (Exception e) {
		...
		try {
			...
			if ((rec.getKey().getName()).compareTo(words[4]) == 0)
				...
				System.out.println("Test 8 failed " + rec.getKey().getName() + ", " + words[4]);
		} catch (Exception e) {
			System.out.println("Test 8 failed " + e);
		}
		...
		try {
			...
			try {
				...
				System.out.println("Test 10 failed " + test10);
			}
